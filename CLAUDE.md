...
**Available Solvers:**
- `algebraSolver.js`: Equations, simplification, factoring (using mathsteps, algebrite). Every mathsteps solution is substituted back into the original equation before it is trusted (mathsteps "factored" x² − 2x − 1 as (x − 1)²); rational equations are cleared of denominators before the exact root path; polynomials in e^x are solved via u = e^x; radicals display as √n.
